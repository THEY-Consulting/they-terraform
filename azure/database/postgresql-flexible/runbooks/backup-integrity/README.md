...
Publish an immutable version tag or digest; Terraform rejects `:latest`. The consumer pipeline must build, vulnerability-scan, generate an SBOM, and push the image before applying Terraform. The Dockerfile follows the maintained `python:3.13-alpine` stream and applies available Alpine security updates at build time. Python dependencies are fully resolved and SHA-256 hash-locked in `requirements.lock`; wheel-only installation prevents source-distribution build hooks. Regenerate the lock file with `uv pip compile requirements.txt --generate-hashes --output-file requirements.lock` when updating dependencies. Rebuild at least monthly and immediately for critical Python/Alpine advisories; the Trivy High/Critical scan must pass without suppressions or severity downgrades before publishing.
